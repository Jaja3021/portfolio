      ...
      <div className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
          {PH_LOCATIONS.map((group) => (
            <div key={group.region} className="rounded-2xl border border-border bg-surface p-6">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {group.provinces.map((province) => (
                  <button
                    ...
                    className="rounded-full border border-border px-3 py-1.5 text-xs font-medium text-foreground/70 transition-colors hover:border-accent hover:text-accent cursor-pointer"
                  >
                    {province}
                  </button>
                ))}
