    ...
    <section className="bg-background">
      <div className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-center">
          <div className="relative mx-auto w-full max-w-md rounded-2xl border border-border bg-muted p-6">
            <svg
              viewBox="0 0 300 500"
              className="w-full"
              role="img"
            ...
            >
              ...
              <path
                ...
                className="fill-accent-light stroke-accent/40"
                strokeWidth="1.5"
              />
              ...
              {PINS.map((pin) => (
                ...
                >
                  <circle
                    ...
                    className="fill-accent stroke-white transition-all"
                    strokeWidth="2"
                  />
                  {active === pin.name && (
                    <circle cx={pin.x} cy={pin.y} r="12" className="fill-accent/20 animate-ping" />
                  )}
                </g>
              ))}
            </svg>
            ...
            {active && (
              <div className="absolute inset-x-6 bottom-6 flex items-center gap-3 rounded-xl border border-border bg-surface p-3 shadow-sm">
                ...
                <div>
                  ...
                  <p className="text-xs text-foreground/60">{PINS.find((p) => p.name === active)!.location}</p>
                </div>
              </div>
            )}
          </div>
            ...
            {PINS.map((pin) => (
              <button
                ...
                className={`flex items-center gap-3 rounded-xl border p-3 text-left transition-colors cursor-pointer ${
                  active === pin.name ? "border-accent bg-accent-light" : "border-border bg-muted hover:border-accent/40"
                }`}
              >
                <span className="flex h-10 w-14 shrink-0 items-center justify-center rounded-lg bg-white">
                ...
                <div>
                  ...
                  <p className="text-xs text-foreground/60">{pin.location}</p>
                </div>
              </button>
            ))}
          </div>
